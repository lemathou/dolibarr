	...
	foreach ($_POST as $key => $value) {
		...
		if (preg_match('/^product_batch_([0-9]+)_([0-9]+)$/i', $key, $reg)) {
			...
			if ($qtytomove > 0) {
				...
				if (!$error) {
					$result = $object->dispatchProduct($user, $productId, $qtytomove, GETPOSTINT($ent), $puformove, GETPOST('comment'), $dDLC, $dDLUO, GETPOST($lot, 'alpha'), GETPOST($fk_commandefourndet, 'int'), $notrigger);
					if ($result < 0) {
						setEventMessages($object->error, $object->errors, 'errors');
						$error++;
					}
			...
			if ($num) {
				...
				if (isModEnabled('productbatch')) {
					...
					if (!getDolGlobalString('PRODUCT_DISABLE_SELLBY')) {
						print '<td class="dispatch_dluo_title">'.$langs->trans("SellByDate").'</td>';
					}
					if (!getDolGlobalString('PRODUCT_DISABLE_EATBY')) {
						print '<td class="dispatch_dlc_title">'.$langs->trans("EatByDate").'</td>';
					}
				} else {
					print '<td></td>';
					print '<td></td>';
			...
			while ($i < $num) {
				...
				if (isModEnabled('productbatch')) {
					if ($objp->batch) {
						...
						if (!getDolGlobalString('PRODUCT_DISABLE_SELLBY')) {
							print '<td class="dispatch_dluo">'.dol_print_date($lot->sellby, 'day').'</td>';
						}
						if (!getDolGlobalString('PRODUCT_DISABLE_EATBY')) {
							print '<td class="dispatch_dlc">'.dol_print_date($lot->eatby, 'day').'</td>';
						}
					} else {
						print '<td class="dispatch_batch_number"></td>';
						if (!getDolGlobalString('PRODUCT_DISABLE_SELLBY')) {
